...
def split_ligands(sdf_file):
    ...
    for mol in suppl:
        ...
        for i, ligand in enumerate(mol.GetMolFrags()):
            ...
            ligand_file = os.path.join(data_path, '{}_ligand_{}.pdb'.format(mol.GetProp('_Name'), i+1))
...
def simulation_openMM(output_dir:str, complex_prmtop: str = 'complex.prmtop', complex_inpcrd: str = 'complex.inpcrd', 
    ...
    checkpointReporter = CheckpointReporter(checkpointReporter_file, checkpointReporter_step)
...
def create_mmpbsa_in(mmpbsa_infile:str, igb:int = 5, number_frames_analysis:int = 10, salt_concentration:float = 0.15):
    if igb == 1:
        mbondi = 'mbondi'
    elif (igb == 2) or (igb == 5):
        mbondi = 'mbondi2'
