...
from helper import count_ligands, create_mmpbsa_in, download_pdb_from_rcsb, generate_complex_tleapin, generate_pdb_4tleap, generate_protein_tleapin, simulation_openMM, split_ligands
...
def protein_process(output_path, input_protein_pdb_path):
    ...
    fix_protein_pdb = os.path.join(output_path, input_protein_pdb_path.split('/')[-1].split('.')[-1] + '_fixed.pdb')
    os.system('pdbfixer {} --output {}'.format(input_protein_pdb_path, fix_protein_pdb))

    # Fix protein pdb
    protein_amber_fix_pdb = os.path.join(output_path, 'protein.fixed.amber.pdb')
    os.system('pdbfixer {} --output {}'.format(fix_protein_pdb, protein_amber_fix_pdb))
    protein_amber_fix_reduceH_pdb = os.path.join(output_path, 'protein.fixed.amber_reduceH.pdb')
    os.system('reduce {} > {}'.format(protein_amber_fix_pdb, protein_amber_fix_reduceH_pdb))
    protein_amber_fix_reduceH_refix_pdb = os.path.join(output_path, 'protein.fixed.amber_reduceH.refix.pdb')
    os.system('pdb4amber -i {} -o {}'.format(protein_amber_fix_reduceH_pdb, protein_amber_fix_reduceH_refix_pdb))
    ...
    generate_protein_tleapin(protein_tleap_in, protein_tleap_pdb, protein_prmtop, protein_inpcrd)
    ...
    protein_prmtop_inpcrd = os.path.join(output_path, 'protein_prmtop_inpcrd.pdb')
    os.system('ambpdb -p {} -c  {} > {}'.format(protein_prmtop, protein_inpcrd, protein_prmtop_inpcrd))
...
def ligand_process(output_path, input_ligand_pdb_path):
    ...
    ligand_frcmod = os.path.join(output_path, 'ligand_wH_4amer__frommol2prepi.frcmod')
...
def calculating_a_pair_protein_ligand(output_path, protein_prmtop_inpcrd, input_ligand_pdb_path):
    ...
    os.system('cat {} {} > {}'.format(protein_prmtop_inpcrd, ligand_amber_pdb, complex_pdb))
    ...
    complex_amber_fix_pdb = os.path.join(output_path, 'complex.fixed.amber.pdb')
    os.system('pdbfixer {} --output {}'.format(complex_amber_pdb, complex_amber_fix_pdb))
    complex_amber_fix_reduceH_pdb = os.path.join(output_path, 'complex.fixed.amber_reduceH.pdb')
    os.system('reduce {} > {}'.format(complex_amber_fix_pdb, complex_amber_fix_reduceH_pdb))
    complex_amber_fix_reduceH_refix_pdb = os.path.join(output_path, 'complex.fixed.amber_reduceH.refix.pdb')
    ...
    complex_coordinate_amber_inpcrd = os.path.join(output_path, 'complex.inpcrd')
    generate_complex_tleapin(complex_tleap_pdb, ligand_prepi, ligand_frcmod,  complex_tleap_in, complex_topology_amber_prmtop, complex_coordinate_amber_inpcrd)
    ...
    simulation_openMM(output_path, complex_topology_amber_prmtop, complex_coordinate_amber_inpcrd,
    ...
    mbondi = create_mmpbsa_in(mmpbsa_infile, args.igb, args.number_frames_analysis, args.salt_concentration)

    Output_name = 'FINAL_RESULTS_MMPBSA.dat'
    final_mmpbsa = os.path.join(output_path, Output_name)

    os.system('ante-MMPBSA.py  -p {} -c com.prmtop -r rec.prmtop -l ligand.prmtop -s :WAT:Na+:Cl-:Mg+:K+ -n :LIG --radii {}'.format(complex_topology_amber_prmtop, mbondi))
    ...
    os.system("MMPBSA.py -O -i {} -o {} -sp {} -cp com.prmtop -rp rec.prmtop -lp ligand.prmtop -y ".format(mmpbsa_infile, final_mmpbsa, complex_topology_amber_prmtop, trajectory_dcd))
...
def main():
    ...
    protein_prmtop_inpcrd = protein_process(output_path, input_protein_pdb_path)
    ...
    if input_ligand_sdf_path is not None:
        if count_ligands(input_ligand_sdf_path) > 1:
            ...
            for input_ligand_pdb_path in ligand_pdb_file_list:
                calculating_a_pair_protein_ligand(output_path, protein_prmtop_inpcrd, input_ligand_pdb_path)
        else:
            input_ligand_pdb_path = input_ligand_sdf_path.replace('.sdf','.pdb')
            os.system('obabel {} -O {}'.format(input_ligand_sdf_path, input_ligand_pdb_path))
            calculating_a_pair_protein_ligand(output_path, protein_prmtop_inpcrd, input_ligand_pdb_path)
    else:
        calculating_a_pair_protein_ligand(output_path, protein_prmtop_inpcrd, input_ligand_pdb_path)
